module Client = struct
  ...
  type t = {
    id : Id.t;
    client : Websocket_lwt_unix.Connected_client.t;
  }
...
module Server = struct
  ...
  type t = {
    ...
    clients :
      ( int,
        Websocket_lwt_unix.Connected_client.t * status ref )
      Hashtbl.t;
  ...
  let run
      { port; next_id; next_id_lock; clients; _ }
      on_connect
      ?on_close
      ?on_client_error
      handler =
    let server =
      ...
      establish_server
        ...
        (fun connected_client ->
          ...
          let client =
            Client.{ id = client_id; client = connected_client }
          in
          let%lwt () =
          ...
          let rec loop () =
            Lwt.catch
              (fun () ->
                let%lwt frame =
                  Connected_client.recv connected_client
                in
                let open Websocket in
  ...
  let broadcast server message =
    Lwt_list.iter_p
      (fun client -> Client.send client message)
      (clients server)
