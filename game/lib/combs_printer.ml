...
(* Doesn't work, when you try to apply this function to a string it demands a format type

let append_to_combos str = 
  let append_printer file str = 
    let o = open_out_gen [Open_append; Open_creat] 0o666 file in
      Printf.fprintf o str;
      close_out o
  in append_printer "./combos.txt" str;;
*)

(* This function is contingent upon append_to_combos being a higher order function

let format_comb a b c d =  
      ...
      for c = 1 to 12 do
        for d = 1 to 12 do
          if [a;b;c;d] |> in_set seen_combs = false && (makes_24 [a;b;c;d]) then
              (
              let o = open_out_gen [Open_append; Open_creat] 0o666 "./combos.txt" in
